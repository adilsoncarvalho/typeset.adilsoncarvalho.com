...
margins reads as a wall, and a reader meets a wall by skimming it.

The running head and the folio are the two marks the engine writes into that
margin without being asked. Neither is typed by the author, and neither may be
allowed to drift from the thing it names: a running head that still announces a
section the reader left four pages ago is worse than no running head at all,
because it is confidently wrong rather than merely absent.

Both are suppressed on the opening page. An opening page announces itself by
being the opening page, and a folio at the foot of it only tells the reader what
they already know. The same argument applies to the head: there is no previous
section for it to name.

The running head is set in the sans at 8pt, uppercased, tracked out by 0.08em
and printed in the faintest of the three inks. Every one of those decisions
pushes it further from the text, which is the point. It is a navigational aid,
consulted perhaps twice in a reading, and it should cost nothing the rest of the
time. The folio is set differently — in the reading face at 9pt, on old-style
figures, in the muted ink — because a page number is read, not scanned, and a
lining figure at the foot of a page shouts.

Neither carries a rule. A rule under a running head is a habit inherited from
