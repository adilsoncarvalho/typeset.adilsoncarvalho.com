...
#paragraphs-indented[
  ...
  marked the start. So it sits flush.

  Subsequent paragraphs indent by 1.5em, with no gap. The page becomes a
  continuous field of text, which is exactly what you want when the reader is
  meant to read every word rather than hunt for one.

  This is the convention of nearly every printed book you own, and it is almost
  never seen on the web — which is the clearest sign that print and screen are
  different media.
]
