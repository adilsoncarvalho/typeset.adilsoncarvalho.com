// A whole document opens with `#show: two-column.with(front: [...])`, which
// sets the page as well as the body. A snippet runs inside a page the
// boilerplate above has already set, so this one calls the body composition
// on its own — the same columns, the same gutter and the same scale.
#two-column-body(
  front: [
