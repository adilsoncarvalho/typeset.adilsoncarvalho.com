...
| `tools/rename-map.json` | The frozen table, derived from the rules above. Read by the codemod and the checker's deprecation gate | Create (committed, then frozen) |
...
**Files:**
- Create: `tools/rename-map.json` (committed, then frozen)

- [x] **Done — commit `05c9284`.**

The old→new table was derived from the naming rules above and committed as
`tools/rename-map.json`. It is frozen: it describes spec 1.0.0 ids, which
`spec.json` no longer carries, so it cannot be re-derived from the current
spec and is corrected by hand when a row is found to be wrong. The derivation,
and the one-shot script that performed it, are in commit `05c9284`.

Two invariants the rest of the plan depends on, both holding in the committed
table: every element id maps to exactly one new id, and no two map to the same
one — a collision would put two styles on one class.
...
The grep is the confirmation that the map was actually applied: every 1.x spelling occurring in the stylesheet must be one the map accounts for.
